...
All responses are cached in Redis for 5 minutes (CACHE_TTL).
...
async def _server_names(db: AsyncSession) -> dict[str, str]:
    result = await db.execute(select(MCPServer.id, MCPServer.name))
...
async def top_tools(
    ...
    cache_key = f"analytics:top_tools:{limit}:{hours}"
    cached = await _cache_get(cache_key)
    if cached is not None:
        return cached

    cutoff = datetime.now(timezone.utc) - timedelta(hours=hours)
    result = await db.execute(select(ToolCall).where(ToolCall.called_at >= cutoff))
    calls = result.scalars().all()
    names = await _server_names(db)

    # Aggregate per (server_id, tool_name)
    agg: dict[tuple, dict] = {}
    for c in calls:
    ...
    for (sid, tname), stats in agg.items():
        ...
        rows.append({
            ...
            "error_rate": (error_count / call_count * 100) if call_count else 0.0,
...
async def error_rates(
    ...
    cache_key = f"analytics:error_rates:{hours}:{server_id}"
    cached = await _cache_get(cache_key)
    if cached is not None:
        return cached

    cutoff = datetime.now(timezone.utc) - timedelta(hours=hours)
    stmt = select(ToolCall).where(ToolCall.called_at >= cutoff)
    if server_id:
...
async def latency_stats(
    ...
    cache_key = f"analytics:latency:{hours}:{server_id}"
    cached = await _cache_get(cache_key)
    if cached is not None:
        return cached
...
async def volume_heatmap(
    ...
    cache_key = f"analytics:volume:{hours}:{bucket_minutes}:{server_id}"
    cached = await _cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    for c in calls:
        # Floor to bucket boundary (normalise naive datetimes from SQLite)
