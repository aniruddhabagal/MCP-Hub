...
export function WorkspaceSwitcher() {
  const { workspace, workspaces, switchWorkspace, canCreateWorkspace } = useAuth()
  ...
  const handleSwitch = async (id: string) => {
    ...
    try {
      ...
      // switchWorkspace errors are surfaced via thrown errors now
    } finally {
      setOpen(false)
    }
  }
            ...
            <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-surface border border-border rounded shadow-lg overflow-hidden">
              ...
              {!isDemoMode() && (
                <>
                  ...
                  <button
                    onClick={canCreateWorkspace ? handleNewWorkspace : undefined}
                    disabled={!canCreateWorkspace}
                    title={canCreateWorkspace ? undefined : 'Leave your current org workspace first'}
                    className={cn(
